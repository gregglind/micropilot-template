...
let showdata = exports.showdata = function(){
  ...
  let data = resolve([]);
  if (_study) {
    micropilot.microlog("study exists! getting data");
    data = _study.data();
  }

  micropilot.microlog(data);
  data.then(function(data){
    console.log("in the then!")
    pageMod.PageMod({
      include: self.data.url("micropilot/data.html"),
      contentScript: 'window.document.getElementById("collecteddata").textContent = JSON.stringify(self.options.data,null,2);',
      contentScriptWhen: 'ready',
      contentScriptOptions:  {data: data}
    });
    console.log("pageMod set")
