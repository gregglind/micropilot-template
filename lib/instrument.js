...
let study = exports.study = function(options){
  let {topics,duration,uploadurl,studyid,
      uploadinterval,demographics,pingurl,killaddon} = options;

  killaddon = killaddon === undefined ? false : killaddon;
  ...
  _study.lifetime(duration).then(function(){
    microlog("end of study lifetime");

    if (options.studyendurl){
      require("tabs").open(options.studyendurl)
    }

    _study.ezupload({
      interval: options.ezuploadretryinterval || 0,
      url: uploadurl,
      killaddon: killaddon
...
let ping = exports.ping = function(url){
  ...
  let R = Request({
    ...
    onComplete: function(response) {
      if (GOODSTATUS[response.status.toString()]) {  // try again in interval ms
...
pageMod.PageMod({
  ...
  contentScript:
    ...
    'window.document.getElementById("copydata").addEventListener("click", function(){self.port.emit("copydata",{data:window.document.getElementById("collecteddata").textContent})},true);',
  ...
  onAttach: function(worker) {
    ...
    worker.port.on("copydata",function(d){
      require("clipboard").set(d.data,"text");
    })
  }
});
